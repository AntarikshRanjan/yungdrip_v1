...
async function buildOrderItems(cartItems) {
  ...
  return cartItems.map((item) => {
    ...
    const stock = getStockForVariant(product, item.size, item.color);

    if (stock !== null && stock < item.quantity) {
      throw new HttpError(400, `${product.name} (${item.size}/${item.color}) only has ${stock} left in stock`);
    }
...
async function decrementInventoryForOrderItems(items) {
  for (const item of items) {
    const product = await Product.findById(item.productId);

    if (!product || !Array.isArray(product.inventory) || product.inventory.length === 0) {
      continue;
    }
    ...
    inventoryEntry.quantity = Math.max(0, inventoryEntry.quantity - item.quantity);
    await product.save();
  }
}
...
export async function finalizeCheckoutOrder({ userId, orderId, razorpayPaymentId, razorpayOrderId, razorpaySignature }) {
  ...
  await sendOrderConfirmationEmail(normalizedOrder).catch(() => {});
...
export async function updateOrderStatusAsAdmin(orderId, nextStatus) {
  ...
  const normalizedOrder = normalizeOrder(order);
  await sendOrderStatusEmail(normalizedOrder, previousStatus).catch(() => {});
...
export async function processRazorpayWebhook({ event, payload }) {
  ...
  if (event === "payment.captured" || event === "order.paid") {
    if (order.payment.status !== "paid") {
      ...
      await order.save();
      await decrementInventoryForOrderItems(order.items);
      await sendOrderConfirmationEmail(normalizeOrder(order)).catch(() => {});
    }
